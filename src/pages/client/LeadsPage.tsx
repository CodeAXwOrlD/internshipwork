...
const getLeadsCacheFromStorage = () => {
  try {
    const uid = localStorage.getItem("last_user_id");
    const cached = uid ? localStorage.getItem(`pixora_leads_cache_${uid}`) : null;
...
let leadsCache: Lead[] | null = getLeadsCacheFromStorage();

const saveLeadsCacheToStorage = (list: Lead[]) => {
  try {
    const uid = localStorage.getItem("last_user_id");
    if (uid) {
      localStorage.setItem(`pixora_leads_cache_${uid}`, JSON.stringify(list));
    }
  } catch {}
};
